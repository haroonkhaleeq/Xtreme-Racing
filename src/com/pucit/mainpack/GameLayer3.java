...
public class GameLayer3 extends CCColorLayer
{
	...
	private float gameSpeed = 2;//2
	private float frameUpdateSpeed = 1;//1
	int time=60;
	...
	public static CCScene scene()
	{
		...
		CCColorLayer layer = new GameLayer3(ccColor4B.ccc4(255, 255, 255, 255));
	...
	protected GameLayer3(ccColor4B color)
	{
		...
		gButton = CCSprite.sprite("green.png");
		gButton.setPosition(CGPoint.ccp( 30.0f, 160 ));
		addChild(gButton);
...
//		addChild(_labelDistance);
	...
	public boolean ccTouchesEnded(MotionEvent event)
	{
		...
		else if (offX>(30.0f-40.0f) && offX<(30.0f+40.0f) && offY<(160+40) && offY>(160-40))
		{
			//Nitrous Boost
			
			CCSprite flame = CCSprite.sprite("flame.png");
			
			if(boost==0)
			{
				
				gameSpeed = 1.0f;
				frameUpdateSpeed = 0.1f;
				
				flame.setPosition(CGPoint.ccp(player.getPosition().x, player.getPosition().y-80));
				this.addChild(flame);
				
				CCMoveTo actionMove = CCMoveTo.action(gameSpeed, CGPoint.ccp(player.getPosition().x, player.getPosition().y-85));
				CCCallFuncN actionMoveDone = CCCallFuncN.action(this, "flameMoveFinished");						
				CCSequence actions = CCSequence.actions(actionMove, actionMoveDone);
				
				flame.runAction(actions);
...
//		_labelDistance.setString("Distance Covered: " + d);			
	...
	public void flameMoveFinished(Object sender)
	{
		CCSprite sprite = (CCSprite)sender;
