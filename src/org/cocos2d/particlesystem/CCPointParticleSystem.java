...
public class CCPointParticleSystem extends CCParticleSystem {
    ...
    public void draw() {
        ...
        gl.glTexEnvi(GL11.GL_POINT_SPRITE_OES, GL11.GL_COORD_REPLACE_OES, GL10.GL_TRUE );

        gl.glBindBuffer(GL11.GL_ARRAY_BUFFER, verticesID[0]);

        gl.glVertexPointer(2, GL10.GL_FLOAT, ccPointSprite.spriteSize*4, 0);

        gl.glColorPointer(4, GL10.GL_FLOAT, ccPointSprite.spriteSize*4, 2*4); // ccPointSprite.color

        gl.glBindBuffer(GL11.GL_ARRAY_BUFFER, verticesID[1]);
        gl.glEnableClientState(GL11.GL_POINT_SIZE_ARRAY_OES);
        gl.glPointSizePointerOES(GL10.GL_FLOAT, 0, sizeBuffer); // ccPointSprite.size
